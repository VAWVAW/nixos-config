{ config, pkgs, lib, ... }: {
  home.persistence."/persist/home/vaw".directories = let
    removePrefix = path:
      builtins.replaceStrings [ config.home.homeDirectory ] [ "" ] path;
  in [
    {
      directory = removePrefix config.accounts.email.maildirBasePath;
      method = "bindfs"; # allow home-manager to manage mailboxes
    }
    {
      directory = removePrefix config.accounts.contact.basePath;
      method = "symlink";
    }
    {
      directory = removePrefix config.accounts.calendar.basePath;
      method = "symlink";
    }
  ];
  ...
  systemd.user.services."sync-dav" = {
    ...
    Service = {
      Type = "oneshot";
      ExecStart = let inherit (config.accounts.calendar) accounts;
      in [
  ...
  accounts = {
    contact.basePath = ".dav/contacts";
    calendar.basePath = ".dav/calendar";
    email.maildirBasePath = "Maildir";
  };
  ...
  programs = {
    ...
    khard = {
      enable = true;
      settings."contact table".localize_dates = "yes";
    };
    khal = {
      enable = true;
      locale = {
