    ...
    in
    {
      ...
      config = rec {
        ...
        window = {
          ...
          titlebar = true;
          commands = [
            { command = "border pixel 2"; criteria = { window_role = "browser"; }; }
            { command = "border pixel 2"; criteria = { class = "Steam"; }; }
            { command = "border pixel 2"; criteria = { app_id = "firefox"; }; }
            { command = "floating enable"; criteria = { title = "Browser$"; }; }
          ];
        };
        assigns = {
          "10" = [{ class = "discord"; }];
        ...
        menu = "${pkgs.bemenu}/bin/bemenu-run --no-exec | xargs swaymsg exec --";
        ...
        keybindings =
          ...
          in
          {
            ...
            # audio
            "XF86AudioRaiseVolume" = "exec --no-startup-id ${pkgs.wireplumber}/bin/wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+; exec pkill -SIGRTMIN+${get_i3block_signal "volume"} i3blocks";
            "XF86AudioLowerVolume" = "exec --no-startup-id ${pkgs.wireplumber}/bin/wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-; exec pkill -SIGRTMIN+${get_i3block_signal "volume"} i3blocks";
            "${mod}+XF86AudioRaiseVolume" = "exec --no-startup-id ${pkgs.wireplumber}/bin/wpctl set-volume @DEFAULT_AUDIO_SINK@ 1%+; exec pkill -SIGRTMIN+${get_i3block_signal "volume"} i3blocks";
            "${mod}+XF86AudioLowerVolume" = "exec --no-startup-id ${pkgs.wireplumber}/bin/wpctl set-volume @DEFAULT_AUDIO_SINK@ 1%-; exec pkill -SIGRTMIN+${get_i3block_signal "volume"} i3blocks";
            ...
            # player control
            "XF86AudioPlay" = "exec --no-startup-id ${pkgs.spotifython-cli}/bin/spotifython-cli play-pause; exec 'sleep 2 && pkill -SIGRTMIN+11 i3blocks'";
            "XF86AudioStop" = "exec --no-startup-id ${pkgs.spotifython-cli}/bin/spotifython-cli pause";
            "XF86AudioNext" = "exec --no-startup-id ${pkgs.spotifython-cli}/bin/spotifython-cli next";
            "XF86AudioPrev" = "exec --no-startup-id ${pkgs.spotifython-cli}/bin/spotifython-cli prev";
