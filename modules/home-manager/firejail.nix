  ...
  wrappedBins = pkgs.runCommandLocal "firejail-wrapped-binaries" {
    ...
    ${concatStringsSep "\n" (mapAttrsToList (command: value:
      let
        ...
        args = escapeShellArgs (opts.extraArgs
...
in {
  options.programs.firejail = {
    firejailBinary = mkOption {
      ...
      description = mdDoc
        "The firejail executable with setuid installed using your package manager";
      ...
      type = types.attrsOf (types.either types.path (types.submodule {
        options = {
          executable = mkOption {
            ...
            example =
              literalExpression ''"''${getBin pkgs.firefox}/bin/firefox"'';
          };
          profile = mkOption {
  ...
  config =
    mkIf (cfg.wrappedBinaries != { }) { home.packages = [ wrappedBins ]; };
