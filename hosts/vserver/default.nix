{ modulesPath, config, ... }: {
  imports = [
    (modulesPath + "/profiles/qemu-guest.nix")
    ../common/optional/networkmanager.nix
    ../common/optional/sslh.nix
    ../common/optional/btrfs-swapfile.nix
    ../common/optional/boot-partition.nix

    ../common/global
    ../common/users/vawvaw

    ./config
  ];

  networking.hostName = "vserver";

  services.sslh.verbose = true;
  services.sslh.listenAddresses = [ "server.vaw-valentin.de" ];

  system.stateVersion = "22.11";

  boot = {
    kernelParams = [ "ip=45.132.244.25::45.132.244.1:255.255.252.0::ens3:off" ];
    initrd = {
      availableKernelModules =
        [ "ata_piix" "uhci_hcd" "virtio_pci" "sr_mod" "virtio_blk" ];
      network = {
        enable = true;
        ssh = {
          enable = true;
          port = 443;
          hostKeys = [ /persist/etc/ssh/ssh_initrd_host_ed25519_key ];
          authorizedKeys = [
            "ssh-rsa  (none)"
          ];
        };
      };
    };
    loader = {
      efi.canTouchEfiVariables = true;
      systemd-boot = {
        enable = true;
        configurationLimit = 25;
      };
    };
  };

  fileSystems."/backed_up" = {
      device = "/dev/disk/by-label/system_partition";
      fsType = "btrfs";
      options = [ "subvol=${config.networking.hostName}/backed_up" "compress=zstd" ];
      neededForBoot = true;
    };

  nixpkgs.hostPlatform.system = "x86_64-linux";
}
